...
export default function App() {
  ...
  const downloadJSON = () => {
    ...
    downloadAnchorNode.setAttribute("download", `glovo_report_${dailyReport.date.replace(/\//g, '-')}.json`);
  ...
  const downloadPDF = () => {
    ...
    doc.setFontSize(22);
    doc.setTextColor(0, 160, 130); // Glovo Green
    ...
    doc.save(`glovo_report_${dailyReport.date.replace(/\//g, '-')}.pdf`);
  };
  ...
  return (
    <div className="flex min-h-screen bg-bg-gray">
      {/* Sidebar */}
      <aside className="w-60 bg-white border-r border-border-gray flex flex-col p-6 shrink-0">
        <div className="text-2xl font-extrabold text-glovo-green tracking-tighter mb-10">
          Incidencias
        </div>
      ...
      <main className="flex-1 p-8 flex flex-col gap-6 overflow-y-auto">
        <header className="flex justify-between items-center">
          <h1 className="text-2xl font-bold tracking-tight">Operaciones: Automatización de Incidencias</h1>
          <div className="flex items-center gap-4">
            <div className="status-badge flex items-center gap-2">
              <div className={`w-1.5 h-1.5 rounded-full ${isLive ? 'bg-glovo-green animate-pulse' : 'bg-red-500'}`} />
              SISTEMA ACTIVO
            </div>
            <button 
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          <div className="card flex flex-col h-[450px]">
            <div className="flex justify-between items-center mb-6 pb-3 border-b border-bg-gray">
              <span className="text-sm font-bold uppercase tracking-wider text-ink-light">A) Flujo en Caliente (Real-Time)</span>
              <span className="text-[11px] font-bold px-2 py-1 bg-blue-50 text-blue-600 rounded">EVENT-BASED</span>
            </div>
                  ...
                  incidents.map((incident) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-[13px] font-semibold truncate">{incident.type.replace('_', ' ')} #{incident.id.slice(0, 4)}</div>
                        <div className="text-[12px] text-ink-light flex items-center gap-1">
                          <MapPin size={10} /> {incident.zone} • {new Date(incident.timestamp).toLocaleTimeString()}
                        </div>
                      </div>
                      <div className="text-[11px] font-bold text-gray-300 group-hover:text-ink-light transition-colors">WEBHOOK</div>
                    </motion.div>
                  ))
                )}
              </AnimatePresence>
          ...
          <div className="card flex flex-col h-[450px]">
            <div className="flex justify-between items-center mb-6 pb-3 border-b border-bg-gray">
              <span className="text-sm font-bold uppercase tracking-wider text-ink-light">B) Resumen Diario Consolidado</span>
              <span className="text-[11px] font-bold px-2 py-1 bg-[#E8F7F3] text-glovo-green rounded">CRON JOB (00:00)</span>
            </div>
            ...
            <div className="flex flex-col gap-6">
                  ...
                  return (
                    <div key={zone} className="space-y-1.5">
                      <div className="flex justify-between text-xs font-medium">
                        <span>{zone}</span>
                        <span className="text-ink-light">{count} incidencias</span>
                      </div>
                      <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                        <motion.div 
                          initial={{ width: 0 }}
                          animate={{ width: `${percentage}%` }}
                          className="h-full bg-glovo-green"
                        />
                      </div>
                    </div>
                  );
              ...
              <div className="pt-4 border-t border-bg-gray flex flex-col gap-3">
                <button 
                  ...
                  className="w-full py-3 bg-glovo-yellow text-ink font-bold text-xs uppercase tracking-widest rounded-lg hover:brightness-95 transition-all disabled:opacity-50 shadow-sm flex items-center justify-center gap-2"
                >
                ...
                {dailyReport && (
                  <div className="grid grid-cols-2 gap-2">
                    <button 
                      ...
                      className="py-2 px-4 bg-white border border-border-gray text-ink font-bold text-[10px] uppercase tracking-wider rounded-lg hover:bg-gray-50 transition-all flex items-center justify-center gap-2 shadow-sm"
                    >
                      <FileJson size={14} />
                      JSON
                    </button>
              ...
              {dailyReport && (
                <motion.div 
                  ...
                  className="p-4 bg-gray-50 rounded-lg border border-border-gray text-xs leading-relaxed italic text-ink-light"
                >
                  "{dailyReport.summary}"
                </motion.div>
              )}
